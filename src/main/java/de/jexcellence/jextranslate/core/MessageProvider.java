...
public final class MessageProvider {
    ...
    public List<Component> getComponents(@NotNull String key, @NotNull String locale,
        ...
        if (includePrefix) {
            ...
            for (String prefixMessage : prefixMessages) {
                ...
                Component parsed = parseMessage(processed);
                prefixComponent = prefixComponent == null ? parsed : prefixComponent.append(parsed);
            }
        }

        for (int i = 0; i < rawMessages.size(); i++) {
            String processed = processPlaceholders(rawMessages.get(i), placeholders);
            Component line = parseMessage(processed);
            // Prepend the prefix INLINE to the first content line so a prefixed
            // message renders as "[Prefix] message" on a single line, instead of
            // the prefix landing on its own line above the message (joinComponents
            // newline-joins separate list entries).
            if (i == 0 && prefixComponent != null) {
                line = prefixComponent.append(line);
            }
            components.add(line);
        }
    ...
    private List<String> getRawMessages(@NotNull String key, @NotNull String locale,
        ...
        Optional<List<String>> crossPlugin = R18nManager.findTranslation(key, locale);
        if (crossPlugin.isPresent()) {
            recordKeyUsageMetric(key, locale);
            return crossPlugin.get();
        }
