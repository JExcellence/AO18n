 ...
 * @param forceLocale           when non-null, all players receive this locale regardless of their
 *                                client language setting; set to {@code null} (default) to honour
 *                                each player's client locale
...
public record R18nConfiguration(
    ...
    /** Returns a copy with the given forced locale (null to honour client locales). */
    @NotNull public R18nConfiguration withForceLocale(@Nullable String locale)                    { return toBuilder().forceLocale(locale).build(); }
