...
public final class R18nManager {
    ...
    private static final ConcurrentHashMap<String, R18nManager> registry = new ConcurrentHashMap<>();
    ...
    /**
     * Returns all currently registered R18nManager instances across all plugins.
     *
     * @return unmodifiable view of the registry values
     */
    @NotNull
    public static Collection<R18nManager> allManagers() {
        return registry.values();
    }

    /**
     * Searches all registered managers for a translation key, returning the
     * first match. The current singleton is tried first for performance.
     *
     * @param key    the translation key
     * @param locale the locale code
     * @return the raw translation lines, or empty if no manager has the key
     */
    @NotNull
    public static Optional<List<String>> findTranslation(@NotNull String key, @NotNull String locale) {
        R18nManager primary = instance.get();
        if (primary != null) {
            Optional<List<String>> result = primary.translationLoader.getRawTranslation(key, locale);
            if (result.isPresent()) return result;
        }
        for (R18nManager mgr : registry.values()) {
            if (mgr == primary) continue;
            Optional<List<String>> result = mgr.translationLoader.getRawTranslation(key, locale);
            if (result.isPresent()) return result;
        }
    ...
    /**
     * Checks whether any registered manager has the given key.
     *
     * @param key    the translation key
     * @param locale the locale code
     * @return true if at least one manager can resolve the key
     */
    public static boolean hasKeyInAny(@NotNull String key, @NotNull String locale) {
        for (R18nManager mgr : registry.values()) {
            if (mgr.translationLoader != null && mgr.translationLoader.hasKey(key, locale)) {
                return true;
            }
        }
        return false;
    ...
    public CompletableFuture<Void> initialize() {
        return CompletableFuture.runAsync(() -> {
            try {
                ...
                initialized = true;
                instance.set(this);
                registry.put(plugin.getName(), this);
    ...
    public static final class Builder {
        ...
        /**
         * Forces all players to receive translations in the given locale,
         * ignoring their client language setting. Pass {@code null} to
         * revert to per-player locale detection (the default).
         *
         * @param locale the locale to force, or null to honour client locales
         * @return this builder
         */
        @NotNull
        public Builder forceLocale(@Nullable String locale) {
            configuration = configuration.withForceLocale(locale);
            return this;
        }
         ...
         *
         * <p>Before construction, the builder checks for a
         * {@code translations/config.yml} file in the plugin's data folder.
         * If the file contains a non-empty {@code force-locale} value, it
         * overrides any programmatic {@link #forceLocale(String)} setting
         * so server administrators have final say.
         *
        ...
        /**
         * Reads {@code translations/config.yml} from the plugin's data folder
         * and applies any settings found there. Extracts the default file from
         * the JAR on first run.
         */
        private void applyTranslationConfig() {
            String dirName = configuration.translationDirectory();
            File translationDir = new File(plugin.getDataFolder(), dirName);
            File configFile = new File(translationDir, "config.yml");

            // Extract default config from JAR if it doesn't exist yet
            if (!configFile.exists()) {
                String resourcePath = dirName + "/config.yml";
                try (InputStream in = plugin.getClass().getResourceAsStream("/" + resourcePath)) {
                    ...
                    plugin.getLogger().log(Level.WARNING,
                            "Could not extract default translations/config.yml", e);
                }
            }
            ...
            String fileForceLocale = yaml.getString("force-locale", "");
            if (fileForceLocale != null && !fileForceLocale.isBlank()) {
                configuration = configuration.withForceLocale(fileForceLocale.trim());
                plugin.getLogger().info(String.format(
                        "Force-locale set to '%s' via translations/config.yml", fileForceLocale.trim()));
            }
        }
    }
}
